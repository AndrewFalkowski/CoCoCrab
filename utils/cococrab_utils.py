...
class CoCoCrab:
    ...
    def __init__(
        ...
        # self.prop2 = prop2  # addition
        # self.prop2_target = 1 if prop2_target == "max" else -1  # addition
        self.alpha = alpha
        self.lr = lr
        self.dopant_threshold = dopant_threshold
        self.compute_device = compute_device
        self.model_0 = load_model(self.prop0)
        # self.model_2 = load_model(self.prop2)  # addition
    ...
    def optimize_comp(self, epochs=100):
        ...
        frac = (
            torch.ones(int(self.src.shape[1]))
            .view(1, -1)
            .to(compute_device, dtype=torch.float, non_blocking=True)
        )

        frac_mask = torch.where(self.src != 0, 1, 0)
        frac_mask = frac_mask.to(compute_device, dtype=torch.float, non_blocking=True)

        # tell Adam to optimize the fraction tensor, ensure requires_grad()
        optimizer = optim.Adam([frac.requires_grad_()], lr=self.lr)

        # maintain set learning rate until the last 25 epochs
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, [epochs - 25], gamma=0.1, last_epoch=-1
        )

        # cycle the learning rate - improves learning over with many elements
        # scheduler = torch.optim.lr_scheduler.CyclicLR(
        #     optimizer, base_lr=0.05, max_lr=0.5, cycle_momentum=False
        # )
        criterion = nn.L1Loss()
        ...
        for epoch in tqdm(range(epochs)):
            ...
            if torch.isnan(soft_frac).any():
                print("==================WARNING!=================")
                print("NaN values detected, stopping optimization!")
                print(f"Optimization failed at epoch: {epoch}")
                print("===========================================")
                break
            ...
            if not self.prop1 == "Loss":
                ...
                (
                    ...
                    prop0_pred,
                    prop0_unc,
                ) = self.model_0.single_predict(self.src, soft_frac)
                loss0 = criterion(
                    scaled_p0,
                    torch.tensor([[self.prop0_target * 100000.0]]).to(compute_device),
                )
                scaled_p1, _, prop1_pred, prop1_unc = self.model_1.single_predict(
                    self.src, soft_frac
                )
                loss1 = criterion(
                    scaled_p1,
                    torch.tensor([[self.prop1_target * 100000.0]]).to(compute_device),
                )
                ...
                scaled_p0, _, prop0_pred, prop0_unc = self.model_0.single_predict(
                    self.src, soft_frac
                )
                loss0 = criterion(
                    scaled_p0,
...
def print_results(optimized_frac_df, prop1="Loss"):
    ...
    print("================================")
    print(
        pd.DataFrame({"Elements": src, "Fractions": frac}).to_markdown(
            index=False, tablefmt="github", numalign="left"
        )
    )
    print("\n")
    print(
        optimized_frac_df.tail(1)
        .iloc[:, 2:5]
        .to_markdown(
            index=False,
            tablefmt="github",
            headers=("prop0 value", "prop0 uncertainty", "prop0 loss"),
        )
    )
    if not prop1 == "Loss":
        print("\n")
        print(
            ...
            .to_markdown(
                index=False,
                tablefmt="github",
                headers=("prop1 value", "prop1 uncertainty", "prop1 loss"),
            )
        )
...
def load_model(prop):
    try:
        ...
        print(
            "It is likely that you are trying to load a property model that is not available."
        )
...
def dopant_filter(vec, threshold):
    ...
    thresh_vec = torch.where(
        norm_vec < threshold, torch.tensor(0.0, dtype=norm_vec.dtype), norm_vec
    )
...
def elem_lookup(src):
    try:
        ...
        numpy_src = src
    # fmt: off
    all_symbols = [ "H", "He", "Li", "Be", "B", "C", "N", "O", "F", "Ne", "Na", "Mg", 
...
def save_results(optim_frac_df, save_dir=None):
    if save_dir is not None:
        delim = "-"
        file_name = (
            f"{save_dir}/{delim.join(optim_frac_df.iloc[0,0])}_optimization_results.csv"
        )
