...
def property_optim_plot(optim_frac_df, prop0, prop1, save_dir=None):
    ...
    plt.tick_params(
        which="minor", direction="in", labelcolor=color, length=4, right=True, top=True
    )
    ...
    ax2.tick_params(axis="y", labelcolor=color, direction="in", length=7)
...
def element_optim_plot(optim_frac_df, save_dir=None):
    ...
    atom_percent = (
        np.concatenate(optim_frac_df["Fractions"].values).reshape(-1, num_elems)
    ) * 100
    ...
    ax1.get_yaxis().set_minor_locator(minor_locator_y)
    plt.tick_params(
        which="minor", direction="in", length=4, right=True, left=True, top=True
    )
    ...
    if save_dir is not None:
        delim = "-"
        fig_name = (
            f"{save_dir}/{delim.join(optim_frac_df.iloc[0,0])}_element_fractions.png"
        )
        os.makedirs(save_dir, exist_ok=True)
        figure = plt.gcf()
        figure.set_size_inches(5, 5)
...
def two_panel_optim(optim_frac_df, prop0, prop1, save_dir):
    ...
    spec = gridspec.GridSpec(
        ncols=2, nrows=1, figure=fig, width_ratios=[1, 1], wspace=0.35
    )
    ...
    ax11.tick_params(axis="y", labelcolor=color, direction="in", length=7)
    minor_locator_x = AutoMinorLocator(2)
    minor_locator_y = AutoMinorLocator(2)
